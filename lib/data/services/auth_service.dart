...
class UserService {
  ...
  Future<void> register({
    ...
    try {
      ...
      await _dio.post('/usuarios', data: requestData);
    } on DioException catch (e) {
      if (e.response != null && e.response?.data != null) {
  ...
  Future<void> updateLanguage(String userId, String languageCode) async {
    try {
      await _dio.patch('/usuarios/$userId', data: {'language': languageCode});
    } on DioException catch (e) {
      if (e.response != null && e.response?.data != null) {
        throw Exception(
          e.response?.data['message'] ?? 'Error actualizando idioma',
        );
      }
      throw Exception('Error de conexión: ${e.message}');
    }
  ...
  Future<void> updateTheme(String userId, String theme) async {
    try {
      await _dio.patch('/usuarios/$userId', data: {'theme': theme});
    } on DioException catch (e) {
      if (e.response != null && e.response?.data != null) {
        throw Exception(
          e.response?.data['message'] ?? 'Error actualitzant tema',
        );
      }
  ...
  Future<User> getMe() async {
    try {
      ...
      if (e.response != null && e.response?.data != null) {
        throw Exception(
          e.response?.data['message'] ?? 'Error obteniendo perfil',
        );
      }
      throw Exception('Error de conexión: ${e.message}');
    }
